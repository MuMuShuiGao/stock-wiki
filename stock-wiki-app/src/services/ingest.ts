...
## Wikilink 引用规则
- 正文中提及**可用 Wikilink 目录**中列出的实体时，必须使用 \`[[type/title]]\` 格式创建链接
- 每个页面正文中至少应有 1–2 个 wikilink（如目录中有相关页面）
- 同一实体在正文中**首次提及时**使用 wikilink，后续提及用纯文本即可，避免满篇重复链接
- wikilink 直接嵌入段落文字中，不带引号，如 \`受益于[[概念/消费电子]]需求复苏\`
- **利用目录中的摘要判断相关性**——摘要不相关的页面不要硬链`;
