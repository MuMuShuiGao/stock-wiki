...
/** 从 index.md + 当前批次页面构建 wikilink 目录，注入 Stage 3/4 prompt */
function buildWikilinkCatalog(indexContent: string, pages: PlannedPage[]): string {
  const existing = new Map<string, string>();
  // 解析 index.md 条目: [[type/title]] — summary
  const re = /\[\[([^\]/|]+)\/([^\]|]+)(?:\|[^\]]+)?\]\]\s*[—–-]\s*(.+)/g;
  let match: RegExpExecArray | null;
  while ((match = re.exec(indexContent)) !== null) {
    existing.set(`${match[1]}/${match[2]}`, match[3].trim());
  }
  ...
  if (existing.size > 0) {
    lines.push("### 已有页面");
    for (const [key, summary] of existing) {
      lines.push(`[[${key}]] — ${summary}`);
    }
  }

  // 同批次 create 页面，filter 掉已在 index 中的，用 rationale 当摘要
  const batchOnly = pages.filter((p) => !existing.has(`${p.type}/${p.title}`));
  if (batchOnly.length > 0) {
    if (lines.length > 0) lines.push("");
    lines.push("### 本轮新建页面（同批次，正文中也可引用）");
    for (const p of batchOnly) {
      const desc = p.rationale || "";
      lines.push(`[[${p.type}/${p.title}]]${desc ? ` — ${desc}` : ""}`);
    }
  }
...
async function parseAndWriteFileBlocks(
  ...
  for (const chunk of chunks) {
    ...
    try {
      ...
      if (bodyIdx === -1) {
        throw new Error(`缺少 ====BODY==== 分隔符（前后必须各有一个空行）`);
      }
...
export async function runStage2Planning(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (lastError.message.includes("API Key") || lastError.message.includes("401")) throw lastError;
    }
  }
...
**裸 JSON**：JSON 部分直接输出裸 JSON 对象，不要包裹在 \`\`\`json\`\`\`、\`---json\`/\`---\`、或 \`---\`/\`---\` 当中

**BODY 分隔符**：\`====BODY====\` 必须独占一行，前后各有一个空行（即 \`\\n\\n====BODY====\\n\\n\`）

**格式校验**：如果缺少 \`====BODY====\` 分隔符，格式校验将报错，该页及同批所有页面均不会写入`;

/** JSON 字段使用说明（Stage 3/4 共用，不含时间戳保留提示） */
const JSON_FIELD_NOTES = `## JSON 字段说明

**字段一致**：与上述字段说明中列出的字段完全一致，只是用 JSON 格式表达

**列表字段**：列表字段（tags / related / sources / aliases / concepts / catalysts）使用 JSON 数组

**字符串值**：字符串值不要有多余的引号或转义（JSON 解析器会自动处理）`;

/** 正文 Markdown 格式规范（Stage 3/4 共用） */
const BODY_FORMAT_BLOCK = `## 正文格式规范

**一级标题**：正文第一行必须是 \`# {title}\`，不可省略、不可写成裸文本

**章节标题**：二级标题（##）上下各空一行，三级标题（###）上方空一行、下方不空行

**段落间距**：段落之间空一行，段落内部不空行、不换行（一段写成连续文字）

**空行限制**：不得出现连续两行以上的空行

**禁止列表前缀**：禁止使用任何列表前缀（\`- \`、\`* \`、\`+ \`、\`1. \` 等）。所有并列陈述必须写成独立段落，用 **加粗标题**：内容的格式承接上文

**无缩进**：行首不得有空格或缩进（Markdown 正文顶格写），每行第一个字符必须从第 1 列开始

## Wikilink 引用规则

**首次提及**：正文中提及可用 Wikilink 目录中列出的实体时，必须使用 \`[[type/title]]\` 格式创建链接

**链接数量**：每个页面正文中至少应有 1–2 个 wikilink（如目录中有相关页面）

**去重**：同一实体在正文中首次提及时使用 wikilink，后续提及用纯文本即可，避免满篇重复链接

**嵌入方式**：wikilink 直接嵌入段落文字中，不带引号，如 \`受益于[[概念/消费电子]]需求复苏\`

**相关性**：利用目录中的摘要判断相关性——摘要不相关的页面不要硬链`;
...
**保留已有内容**：不要删除任何现有段落、数据、分析

**只能追加和细化**：新信息作为新段落或子章节添加，或作为对现有段落的补充

**保持格式一致**：沿用已有页面的 Markdown 结构风格

**去重**：如果新信息与已有内容重复，跳过（不要写入重复信息）
...
**其余已有字段**：其余已有字段（如 updated、last_reviewed）更新为当前时间

**sources 字段**：sources 字段必须累积合并——从已有页面的 JSON frontmatter 中读取已有 sources 数组，所有旧值全部保留，末尾追加本次源文件名（去重），不要用新值覆盖
...
interface BatchWriteOptions {
  ...
  indexContent: string;                // index.md 文本，用于构建 wikilink 目录
  catalogPages: PlannedPage[];         // 本轮全部页面（create + update），用于 wikilink 目录
}

async function batchWritePages(opts: BatchWriteOptions): Promise<BatchWriteResult> {
  const { stage, pages, promptTemplate, keepExistingTimestamps, userMessage, analysisText, sourceFileName, projectName, existingContents, indexContent, catalogPages } = opts;

  logInfo(stage, `批量写入: ${pages.length} 页: ${pages.map((p) => p.title).join(", ")}`);

  const nowStr = formatTimestamp(new Date());
  const wikilinkCatalog = buildWikilinkCatalog(indexContent, catalogPages);
  ...
  const basePrompt = promptTemplate
    ...
    .replace("{WIKILINK_CATALOG}", wikilinkCatalog)
